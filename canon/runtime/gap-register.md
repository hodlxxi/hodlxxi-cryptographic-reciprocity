...
| Minimum Obligation Object v0 | Paid jobs, requests, receipts, invoices, verification endpoint; evidence boundary metadata is specified as a required interpretive dependency. | Missing as first-class runtime mechanism | Paid job, receipt, timestamp, or agent execution proves obligation fulfilled, satisfaction, or accepted outcome. | Keep runtime wording as event proof only; future obligation records should link evidence through evidence boundary metadata and distinguish created, accepted, due, execution, and verification time. |
| Minimum Delegation Record v0 | Agent identity surfaces, operator key, capabilities, signed messages, requester keys; Canon mechanism now requires evidence boundary references, identity/key-control boundaries, time/order semantics, and obligation links when delegated action creates or touches obligations. | Missing as first-class mechanism | Capability, key control, operator key, signed message, agent fluency, or paid task proves delegated authority. | Define read-only delegation record or discovery surface before delegated action claims; require evidence boundary metadata and obligation-object references for delegated obligation effects. |
| Repair Lifecycle v0 | Trust events, attestations, receipts, job status/history, evidence surfaces, paid jobs, delegation-adjacent agent activity, and reputation memory. Canon mechanism now requires evidence boundary references, time/order semantics, obligation links where payment/refund/breach/remedy/satisfaction is involved, and delegation links where delegated authority is involved. | Missing as first-class mechanism | Refund, apology, closure, attestation, repair receipt, or reputation update proves repair, forgiveness, justice, restored trust, obligation satisfaction, or accountable delegated-agent remediation. | Define repair/dispute lifecycle before reputation restoration or forgiveness claims; require Evidence Boundary Metadata v0 for repair evidence, Minimum Obligation Object v0 for obligation-related repairs, and Minimum Delegation Record v0 for delegated-agent misuse. |
| Reciprocity Pattern v0 | Repeated events, paid jobs, messages, marketplace interactions, trust events; Canon mechanism now requires evidence boundary references, time/order semantics, obligation links where obligations affect the pattern, delegation links where agents act for counterparties, and repair lifecycle links where failures or re-entry affect reciprocity. | Missing as first-class mechanism | Transaction count, repeated interaction, equal sats, payment history, reputation score, repair record, or agent activity proves reciprocity, fairness, trust, or restored cooperation. | Represent event sequences with counterparties, roles, bounded evidence, time horizon, sequence context, expectations, asymmetries, contributions, benefits, burdens, delayed returns, non-monetary actions, dispute/repair/re-entry context, reputation context, explicit non-claims, and human interpretation requirements before reciprocity claims. |
